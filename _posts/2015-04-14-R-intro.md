...
We will start with an interactive tutorial called [swirl](http://swirlstats.com/).  The first step is to install swirl.  This only needs to be done once per computer by entering the following text in the Console pane of RStudio.
...
Complete the `swirl()` tutorials before continuing. You might want to jot down any helpful notes from this tutorial.
...
Before we continue, let's start our script file with some information about what it is.
...
```
Note that in the above snippet I again used the comment character "#" (read out loud as hash or hash symbol).  Any text that follows a "#" is considered a comment and is not interpreted.  I highly recommend extensively commenting your code to explain what you are doing.  What makes sense today will seem like gibberish tomorrow.  Comments can help.
...
In the `swirl()` tutorial you learned how to use the brackets [] to extract elements from objects.  Let's practice here.

**Exercise 4:** What is the sum of the 5th through 10th element inclusively of object b?  Provide your code and the sum. 
...
You were also introduced to 2-dimensional objects in `swirl()`.  Let's get a bit more practice with them here.  The matrix command makes multi-dimensional objects.  When extracting from a multi-dimensional object you must specify both dimensions within the square brackets.
...
Now that you have some R fundamentels let's deploy them on the blast results from Ian's Section

### Data import
The easiest way to bring data into R is in a .csv(comma-separated values) or .tsv (tab-separated values) files.  Today we already have the file in the correct format.  In the future if you need to import other data into R, both of these formats can be created in Excel (choose "Save As..." in Excel and select the correct format).  The relevant R import functions are `read.delim()` and `read.csv()` or `read.table()`.
...
#did you tab-complete that file path?
#the header=F argument tells R that the first row contains data, not column names
#this might take a while to read in this large data set.
```
Look in the Environment pane and you can see a brief description of the data. There are 204351 rows and 22 columns.  It is important that you look at files after they have been read in with the `head()` and `summary()` functions to quality control your work and save yourself from headaches later on.
...
**Excercise 9**:
What is different about those hits with an e-value of 0 and a percent identity less than 50, as compared to those that have an e-value of 0 but a percent identity greater than or equal to 50? Show any relevant code used to answer this question.
...
## Intro to plotting
One of the strengths of R is its graphics.  We are going to take a break from the bioinformatics datasets and work on a tutorial made by our a former TA Ciera.  This dataset uses the "stereotypes.xls" file available on SmartSite.  
...
Now let's apply what we have learned to the BLAST data set.
