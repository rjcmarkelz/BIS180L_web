...
The [tutorial at Rstudio](http://shiny.rstudio.com/tutorial/) is good but pretty involved.  I have tried to distill some of the principles below; if you like it and want more then feel free to do the [tutorial](http://shiny.rstudio.com/tutorial/) as well.
...
* _shinyUI_ This is the overall function that creates that user interface
  ...
  * _sidebarLayout_ Creates a sidebar layout within fluidpage
    ...
    * _mainPanel_ specifics that we want a main panel as well
...
* _shinyServer_ this creates the server application.
  * _renderPlot_ this tells R that the enclosed code will generate a plot that is to be put on the webpage.  Note that there are similar functions for outputting tables and text.
...
```
## Error in shinyAppDir(x): No Shiny application exists at the path "PATH_TO_APP_DIRECTORY"
```
...
* Each user should clone it __to a directory outside of the "BIS180L_assignments"__ directory.  Sorry about that but we do not want nested repositories.
