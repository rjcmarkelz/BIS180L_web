...
Remember last week when we were concerned with what distribution the RNA-seq count data was coming from so that we could have a good statistical model of it? Well, if we want to perform good clustering we also need to think about this because most of the simplest clustering assumes data to be from a normal distribution. I have precalculated this for you.
...
## 	/var/folders/jh/6yqw6n710sj2xr9knz37tt0w0000gn/T//RtmpGa5Z1N/downloaded_packages
```
...
**Exercise 6:** The genes are definitely overplotted and we cannot tell one from another. However, what is the most obvious pattern that you can pick out from this data? Describe what you see.
...
Now that you have a sense for how this k-means works, lets apply what we know to our data. There are a few convenience libraries that we will be loading here so you do not have to mess around with the ggplots, but focus on what the k-means is trying to accomplish with the data. Remember that if you do not have a library installed use *install.packages()*. 
...
Now we want to use this package to visualize the k-means clusters. Lets start with 9 clusters, but please play with the number of clusters and see how it is visualized.
...
Now to load one more library for the day to do some further calculations and visualizations for us. 
...
**Exercise 8:** Pretty Colors! Describe what you see visually with 2, 5, 9, and 15 clusters using either method. Why would it be a bad idea to have to few or to many clusters? Discuss with a specific example comparing few vs. many. Justify your visual choice of too many and too few clusters.
