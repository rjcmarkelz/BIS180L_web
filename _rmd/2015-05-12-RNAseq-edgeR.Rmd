...
This has created a design matrix where each row represents one sample and each column represents a factor in the experiment.  The "1"s indicate that a particlar factor applies to a sample.  In this design we have a coefficient to indicate the genotype and a coefficient to indicate the treatment.
...
It would be handy to plot bar charts of genes of interest.  Below is a function to do that.

```{r plotDE_function}
plotDE <- function(genes, dge) {
  tmp.data <- log2(cpm(dge[genes,])+1)
  colnames(tmp.data) <- dge.data$samples$group
  pl <- ggplot(melt(tmp.data),aes(x=Var2,y=value))
  pl <- pl + facet_wrap( ~ Var1)
  pl <- pl + ylab("log2(cpm)") + xlab("group")
  pl <- pl + geom_boxplot()
...
```{r plotDE}
# A single gene
plotDE("Bra009785",dge.data)

#top 10 genes
plotDE(rownames(DEgene.gt)[1:10],dge.data)
```
...
__c__.  Make a plot of the top 10
...
#top 10 genes
plotDE(rownames(DEgene.trt)[1:10],dge.data)
