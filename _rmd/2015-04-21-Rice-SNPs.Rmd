...
## Getting Started
* I have created a directory in your `BIS180L_Assignments` repository called `Assignment_4` and have placed an `Assignment_4.Rmd` template there.  
...
data.pheno <- read.csv("../data/RiceDiversity.44K.MSU6.Phenotypes.csv",row.names=1)
head(data.pheno)
summary(data.pheno)
```
...
The second component, AmID, shows an assigment of each individual to a single ancestral population.  Of course this can be somewhat problematic for heaviliy admixed individuals.
