...
In Thursday's lab we finished by making a .vcf file.  I realized that we need to add a couple of additional flags to `samtools mpileup` to get all of the information that we need about these polymorphisms (Thursday's lab is now updated).  Unless you have already run `samtools mpileup` with the -DV flags, please re-run it as follows:
