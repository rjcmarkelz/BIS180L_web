...
```{r hello_world_md, echo=FALSE,comment=FALSE} 
cat("```{r hello_world2}")
cat("Hello World") 
cat("```")
```
...
```{r Ex8 Answers, echo=KEY, eval=KEY}
sum(fly.worm$E_value==0) #892
sum(fly.worm$percent_ident>50) #17561
sum(fly.worm$E_value==0)/nrow(fly.worm)*100 # .43%
sum(fly.worm$percent_ident>50)/nrow(fly.worm)*100 #8.6%
sum(fly.worm$E_value==0 & fly.worm$percent_ident<50) #337
min(fly.worm$percent_ident[fly.worm$E_value==0]) 34.19
