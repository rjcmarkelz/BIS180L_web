...
First we must get the sequence of the promoters.  For ease we will define the promoters as the 1000bp upstream of the startof the gene.  In the interest of time I have precomputed this for you, but I show you how to do this below in case you need to do it in the future.
...
First lets gather the data that we need
...
Siobhan Brady has compiled a file of plant transcrition factor binding motifs.  You can download those from
...
motifEnrichment <- function(target.promoters,universe.promoters,label=NULL,all.counts=F,motifs=motifsSS) {
  ...
  results.table <- cbind(names(motifs),round(universe.counts.sum/dim(universe.counts)[2],3)*100,
                         round(target.counts.sum/dim(target.counts)[2],3)*100,
                         results)
  colnames(results.table) <- c("motif",paste(label,c("%universe","%target","p.value"),sep="."))
  results.table <- results.table[order(results.table[,paste(label,"p.value",sep=".")]),]
  results.table
}
```
...
```{r enrich_finally}
motif.results <- motifEnrichment(target.promoters,universe.promoters,"Interaction_DEgenes")
head(motif.results)
```
...
__c.__ What percentage of genes in the "Universe" have this motif?  What perventage in our target set?  Do you think this is a biologically meaningful result?  Discuss why or why not.
